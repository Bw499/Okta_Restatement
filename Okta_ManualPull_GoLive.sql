...
#DECLARE DetectionDate DateTime = DateTime.Parse("2023-10-31");
...
#IF ("@@SliceStartTime@@".StartsWith("@@"))
    #DECLARE SliceStartTime string = "2023-10-31";
#ELSE
...
// Get detection data (manually uploaded using the Excel file shared by Sita)
...
//// Join Detection data and TenantProfile together -- Total TPID, Okta Detection 
//Detection =
//    SELECT a.TPID,
//           IF(a.TPID == b.TPID, true, false) AS IsTPIDOkta
//    FROM TenantTPId AS a
//    LEFT OUTER JOIN
//        detection AS b
//        ON a.TPID == b.TPID;
...
output =
    SELECT TenantTPId.TPID,
           ...
           IsF500,
           IsG500,
           IF(TenantTPId.TPID == detection.TPID, true, false) AS IsTPIDOkta,
           AADPPAU,
           IsAADPPAU,
           IsAADPPAU25,
           IsAADPPAU250,
    ...
    LEFT OUTER JOIN
        detection
        ON TenantTPId.TPID == detection.TPID
    LEFT OUTER JOIN
        FortuneFlagsProfile
...
#DECLARE OutputStream1 string = string.Format("/local/users/beca/Okta/new_managed_Okta_{0}_{1}_{2}.ss", @windowStart.Year.ToString(), @windowStart.Month.ToString("D2"), @windowStart.Day.ToString("D2"));
OUTPUT output
...
#DECLARE OutputStream4 string = string.Format("/local/users/beca/Okta/agg_managed_Okta_{0}_{1}_{2}.ss", @windowStart.Year.ToString(), @windowStart.Month.ToString("D2"), @windowStart.Day.ToString("D2"));
OUTPUT output_agg
TO SSTREAM @OutputStream4;
